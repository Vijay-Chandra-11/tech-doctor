...
// // import { getScores } from "@/lib/gameStore";
// // import { StudentScore } from "@/lib/gameData";
// // import { LogOut, RefreshCw, Trophy, Clock, Activity, Star, Layers } from "lucide-react";
...
// //   // FIXED: Made this async so it waits for Firebase
// //   async function loadScores() {
// //     try {
// //       const data = await getScores();
      
// //       // Sort by Level (Highest first), then by Score (Highest first)
// //       data.sort((a, b) => {
// //         // Fallback to level 1 if old data from your friend's DB doesn't have a level
...
// //           transition={{ delay: 0.4 }}
// //         >
// //           {/* UPDATED Table header with Level column */}
// //           <div className="grid grid-cols-6 gap-4 px-6 py-3 text-xs font-bold tracking-widest uppercase"
// //             style={{
// //               background: "linear-gradient(90deg, hsla(186,100%,50%,0.1), hsla(300,100%,60%,0.1))",
// //               borderBottom: "1px solid hsla(186,100%,50%,0.2)",
// //               fontFamily: "'Orbitron', sans-serif",
...
// //             <div className="flex items-center gap-1"><Trophy size={12} /> Rank</div>
// //             <div className="flex items-center gap-1"><Star size={12} /> Student</div>
// //             <div className="flex items-center gap-1"><Layers size={12} /> Lvl</div>
// //             <div className="flex items-center gap-1"><Clock size={12} /> Time</div>
// //             <div className="flex items-center gap-1"><Activity size={12} /> Organs</div>
// //             <div className="text-right">Score</div>
// //           </div>
...
// //               <motion.div
// //                 key={s.name + i}
// //                 className="grid grid-cols-6 gap-4 px-6 py-4 items-center border-b"
// //                 style={{ borderColor: "hsla(186,100%,50%,0.08)" }}
// //                 initial={{ opacity: 0, x: -20 }}
// //                 animate={{ opacity: 1, x: 0 }}
// //                 transition={{ delay: 0.05 * i + 0.5 }}
...
// //                 </div>
// //                 {/* Level (NEW) */}
// //                 <div className="font-bold text-sm" style={{ color: "hsl(var(--neon-magenta))", fontFamily: "'Rajdhani', sans-serif" }}>
// //                   Lv. {s.level || 1}
// //                 </div>
// //                 {/* Time */}
// //                 <div className="font-mono text-sm" style={{ color: "hsl(var(--neon-cyan))" }}>
...
//       // FIXED: Sorting Logic -> Organs (High) > Score (High) > Time (Low)
//       data.sort((a, b) => {
//         // 1. Sort by Organs Restored (Highest first)
//         if (b.organsRestored !== a.organsRestored) {
//           return b.organsRestored - a.organsRestored;
//         }
//         // 2. If Organs are tied, sort by Score (Highest first)
//         if (b.totalScore !== a.totalScore) {
//           return b.totalScore - a.totalScore;
//         }
//         // 3. If Score is also tied, sort by Time Taken (Lowest first / Fastest)
...
//   useEffect(() => {
//     loadScores();
//     const interval = setInterval(loadScores, 10000); // Auto-refresh every 10s
//     return () => clearInterval(interval);
//   }, []);
...
//           transition={{ delay: 0.4 }}
//         >
//           {/* Table header (Reverted to grid-cols-5) */}
//           <div className="grid grid-cols-5 gap-4 px-6 py-3 text-xs font-bold tracking-widest uppercase"
//             style={{
//               background: "linear-gradient(90deg, hsla(186,100%,50%,0.1), hsla(300,100%,60%,0.1))",
//               borderBottom: "1px solid hsla(186,100%,50%,0.2)",
...
export default function Leaderboard({ onLogout }: LeaderboardProps) {
  ...
  async function loadScores() {
    try {
      ...
      // THE RULE OF THREE: Organs (High) > Score (High) > Time (Low)
      data.sort((a, b) => {
        // 1. Sort by Organs Restored (Highest first)
        if (b.organsRestored !== a.organsRestored) {
          return b.organsRestored - a.organsRestored;
        }
        // 2. If Organs are tied, sort by Score (Highest first - Supports Negatives!)
        if (b.totalScore !== a.totalScore) {
          return b.totalScore - a.totalScore;
        }
        // 3. If Score is also tied, sort by Time Taken (Lowest first / Fastest)
  ...
  return (
    <div className="relative min-h-screen flex flex-col overflow-hidden"
      ...
      <div className="flex-1 p-6 max-w-5xl mx-auto w-full">
        ...
        {/* Full table */}
        <motion.div className="glass-card rounded-2xl overflow-hidden glow-border-cyan" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
          <div className="grid grid-cols-5 gap-4 px-6 py-3 text-xs font-bold tracking-widest uppercase"
            style={{
              background: "linear-gradient(90deg, hsla(186,100%,50%,0.1), hsla(300,100%,60%,0.1))",
              borderBottom: "1px solid hsla(186,100%,50%,0.2)",
