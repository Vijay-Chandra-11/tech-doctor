...
// export function QuestionModal() {
//   const { 
//     activePart, setActivePart, curePart, failPart, addPenalty,
//     partProgress, updatePartProgress 
//   } = useGameStore();
...
//   // Pull the current progress for THIS specific organ from the global memory
//   const currentProgress = activePart ? partProgress[activePart] : null;
//   const questions = currentProgress?.questions || [];
//   const currentQIndex = currentProgress?.currentIndex || 0;
//   const correctCount = currentProgress?.correctCount || 0;

//   useEffect(() => {
//     if (!activePart) return;

//     // IMPORTANT: If we already have progress saved for this organ, DO NOT fetch new questions! Just resume.
//     if (currentProgress) {
//       setIsLoading(false);
//       return;
//     }

//     // Otherwise, fetch new questions and start a new surgery
//     const difficulty = difficultyMap[activePart];
//     setIsLoading(true);
...
//         // Save the newly fetched questions to the store memory for this organ
//         updatePartProgress(activePart, {
...
//         toast({ title: "Error loading questions", variant: "destructive" });
//       });
//   }, [activePart, currentProgress, updatePartProgress]);

//   const handleSubmit = () => {
//     if (!userInput.trim() || !activePart) return;
...
//     if (currentQIndex < 4) {
//       // Move to next question AND save progress to global memory
//       updatePartProgress(activePart, {
//         questions,
...
export function QuestionModal() {
  const { 
    activePart, setActivePart, curePart, failPart, addPenalty,
    partProgress, updatePartProgress, updateScore
  } = useGameStore();
  ...
  }, [activePart, currentProgress, updatePartProgress, toast]);
  ...
  const handleSubmit = () => {
    ...
    if (isCorrect) {
      updateScore(500); // INSTANT +500
      toast({ title: "Correct! +500 Points", variant: "default", style: { border: "1px solid hsl(var(--neon-green))" } });
    } else {
      addPenalty(1); 
      updateScore(-200); // INSTANT -200
      toast({ title: "Wrong! -200 Points", variant: "destructive" });
    }
  ...
  const evaluateOrgan = (finalCorrectCount: number) => {
    if (finalCorrectCount === 5) {
      updateScore(500); // FLAWLESS BONUS
      curePart(activePart!);
      toast({ 
        title: "Flawless Surgery! +500 Bonus", 
        description: "5/5 Correct. Organ Perfectly Cured!",
        style: { border: "1px solid hsl(var(--neon-green))" }
      });
