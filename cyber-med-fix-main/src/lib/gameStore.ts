...
//   // NEW: Saves the exact questions and progress for each specific body part
//   partProgress: Record<string, { questions: any[], currentIndex: number, correctCount: number }>;
//   updatePartProgress: (part: string, data: any) => void; 
  
//   setActivePart: (part: string | null) => void;
//   curePart: (part: string) => void;
//   failPart: (part: string) => void; 
//   addPenalty: (amount: number) => void;
//   tick: () => void; 
//   resetGame: () => void;
// }
...
//       penalties: 0,
//       secondsElapsed: 0,
//       partProgress: {}, // Initialize empty
      
//       // Updates the memory for a specific organ
//       updatePartProgress: (part, data) => set((state) => ({
//         partProgress: { ...state.partProgress, [part]: data }
//       })),
...
//         penalties: 0, 
//         secondsElapsed: 0,
//         partProgress: {} // Wipes progress completely for the next team
//       })
//     }),
//     {
//       name: 'tech-doctor-storage',
//     }
//   )
// );
...
// Saves to Firebase Database using the "Rule of Three" Leaderboard Logic
export async function saveScore(score: StudentScore) {
  try {
    const scoreRef = doc(db, "scores", score.name);
    const docSnap = await getDoc(scoreRef);
    ...
    if (docSnap.exists()) {
      ...
      // RULE OF THREE: 1. Organs > 2. Score > 3. Time
      const isBetter = 
        score.organsRestored > existingScore.organsRestored || 
        (score.organsRestored === existingScore.organsRestored && score.totalScore > existingScore.totalScore) ||
        (score.organsRestored === existingScore.organsRestored && score.totalScore === existingScore.totalScore && score.timeTaken < existingScore.timeTaken);
